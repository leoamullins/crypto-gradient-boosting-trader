...
sigs = build_signals_regression(aligned, predictions, regime, breakout_n=BREAKOUT_N, ensemble_scoring=True)

stats = run_backtest(
    sigs, H=H, roundtrip_cost_logret=COST,
    use_stops=False, use_position_sizing=True, max_position=1.0
)
...
ax1.fill_between(range(len(cumulative_pnl)), 0, cumulative_pnl.values, alpha=0.3, color='#2E86AB')
...
ax1.text(0.02, 0.95, f'Final P&L: {final_pnl:.6f}',
...
ax2.hist(winners, bins=50, alpha=0.7, color='green', label=f'Winners ({len(winners)})', edgecolor='black')
ax2.hist(losers, bins=50, alpha=0.7, color='red', label=f'Losers ({len(losers)})', edgecolor='black')
ax2.axvline(x=pnl.mean(), color='blue', linestyle='--', linewidth=2, label=f'Mean: {pnl.mean():.6f}')
...
bp = ax3.boxplot(data_to_plot, labels=['Winners', 'Losers'], patch_artist=True,
...
ax3.text(0.5, 0.02, f'Win Rate: {win_rate:.1f}%\nW/L Ratio: {wl_ratio:.2f}',
...
wedges, texts, autotexts = ax4.pie(exit_counts.values, labels=exit_counts.index,
...
if len(pnl) > window:
    ...
    ax5.fill_between(range(len(rolling_sharpe)), 0, rolling_sharpe.values,
                      where=(rolling_sharpe.values > 0), alpha=0.3, color='green')
    ax5.fill_between(range(len(rolling_sharpe)), 0, rolling_sharpe.values,
                      where=(rolling_sharpe.values <= 0), alpha=0.3, color='red')
    ax5.set_title(f'Rolling Sharpe Ratio ({window}-trade window)', fontsize=12, fontweight='bold')
...
if 'position_size' in stats.columns:
    ...
    ax6.axvline(x=position_sizes.mean(), color='red', linestyle='--',
                linewidth=2, label=f'Mean: {position_sizes.mean():.3f}')
...
ax7.annotate(f'Max DD: {max_dd:.6f}',
             xy=(max_dd_idx, max_dd), xytext=(max_dd_idx + 100, max_dd + 0.005),
...
ax8.text(0.1, 0.9, summary_text, fontsize=11, verticalalignment='top',
         family='monospace',
         bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.3))

# Overall title
fig.suptitle('Crypto Gradient Boosting Trader - Performance Analysis',
